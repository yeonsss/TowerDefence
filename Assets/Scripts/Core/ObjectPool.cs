...
namespace Core
{
    ...
    public class ObjectPool
    {
        ...
        private GameObject CreateInstance(string path, PoolType type)
        {
        ...
        public GameObject GetInstance(string name, PoolType type)
        {
            ...
            if (activeOs == null)
            {
                switch (type)
                {
                    case PoolType.EnemyPool :
                        return CreateInstance($"Enemy/{name}", type);
                    case PoolType.ProjectilePool :
                        return CreateInstance($"Projectile/{name}", type);
                    case PoolType.TowerPool :
                        return CreateInstance($"Tower/{name}", type);
                    default:
